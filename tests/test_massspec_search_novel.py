...
def test_classify_non_reference() -> None:
    ...
    # Pass an aligned_to explicitly set to anything other than
...
def test_read_mmseqs_tab_basic(tmp_path: Path) -> None:
    ...
    table = read_mmseqs_tab(tab, aligned_to="reference")
    ...
    assert table.column("aligned_to").to_pylist() == ["reference"] * 3
...
def test_read_mmseqs_tab_no_tier(tmp_path: Path) -> None:
    """aligned_to left null when caller doesn't specify."""
    tab = tmp_path / "alignments.tab"
    tab.write_text("Q\tT\t1e-50\t1\t10\t1\t10\t10M\n")
    table = read_mmseqs_tab(tab)
    assert table.column("aligned_to").to_pylist() == [None]
...
def test_read_mmseqs_tab_ninth_tier_column(tmp_path: Path) -> None:
    """A 9th aligned_to column (pre-tiered .tab) is read directly,
    ...
    table = read_mmseqs_tab(tab)
    assert table.num_rows == 2
    assert table.column("aligned_to").to_pylist() == [
        "reference", "non_reference",
    ]


def test_read_mmseqs_tab_header_and_tier(tmp_path: Path) -> None:
    """Header + 9th aligned_to column together — the exact shape the
    lab's pre-tiered .tab files have (regression for the OSC failure)."""
    tab = tmp_path / "alignments.tab"
    tab.write_text(
        "query\ttarget\tevalue\tqstart\tqend\ttstart\ttend\tcigar\taligned_to\n"
        "NOVEL1\tREF1\t1.2e-30\t1\t100\t1\t100\t100M\treference\n"
        "NOVEL2\tSWISS_X\t9.9e-18\t5\t50\t1\t46\t46M\tnon_reference\n"
    )
    table = read_mmseqs_tab(tab)
...
def test_read_mmseqs_tab_quoted_header_and_fields(tmp_path: Path) -> None:
    ...
    assert table.column("aligned_to").to_pylist() == ["reference"]
...
def test_tier_inference_marks_reference_targets_as_reference() -> None:
    """When aligned_to is null on input, the classifier infers
    ...
    alignments = pa.table(
        {
            ...
            "tend": [14],
            "cigar": ["14M"],
            "aligned_to": pa.array([None], type=pa.string()),
        }
    )
    detected = pa.table(
    ...
    # aligned_to should be the inferred "reference" (target REF1 ∈ ref proteome)
    tier_vals = result.column("aligned_to").to_pylist()
...
def test_best_hit_tie_break_prefers_reference_over_non_reference() -> None:
    ...
    alignments = pa.table(
        {
            ...
            "tend": [14, 14],
            "cigar": ["14M", "14M"],
            "aligned_to": pa.array([None, None], type=pa.string()),
        }
    )
    ...
    assert result.column("aligned_to").to_pylist() == ["reference"]
...
def test_foreign_tier_vocabulary_recomputed_from_membership() -> None:
    ...
    alignments = pa.table(
        {
            ...
            "aligned_to": pa.array(["refseq", "swissprot"], type=pa.string()),
        }
    )
    detected = pa.table(
    ...
    tiers = {
        r["peptide_sequence"]: r["aligned_to"] for r in result.to_pylist()
    }
    ...
    alignments = pa.table(
        {
            ...
            "cigar": ["14M"], "aligned_to": pa.array([None], type=pa.string()),
        }
    )
    detected = pa.table({"peptide_sequence": pa.array(["LLLLLPPGR"])})
    result = classify_novel_peptides(detected, alignments, reference, novel)
    ...
    alignments = pa.table(
        {
            ...
            "aligned_to": pa.array(["reference", "reference"]),
        }
    )
...
# ──────────────────────────────────────────────────────────────────────
# Phase 5 additions — transcript multi-mapping columns + SwissProt gene
# regression
# ──────────────────────────────────────────────────────────────────────


def test_transcript_id_and_supporting_transcripts_capture_multi_mapping() -> None:
    """When multiple novel ORFs produce the same peptide, the output
    row's `transcript_id` is the winning (most-abundant) ORF, while
    `n_transcripts_supporting` counts all source ORFs and
    `supporting_transcripts` lists them semicolon-joined."""
    pep = "PEPTIDEK"
    detected = pa.table({"peptide_sequence": pa.array([pep])})
    novel = pa.table(
        {
            "protein_id": pa.array(["P_TOP", "P_MID", "P_LOW"]),
            "sequence": pa.array([pep, pep, pep]),
        }
    )
    reference = pa.table(
        {
            "protein_id": pa.array(["R1"]),
            "sequence": pa.array(["PEPTIDK"]),  # 7 aa, insertion vs novel
        }
    )
    alignments = pa.table(
        {
            "query":  pa.array(["P_TOP", "P_MID", "P_LOW"]),
            "target": pa.array(["R1", "R1", "R1"]),
            "evalue": pa.array([1e-30, 1e-30, 1e-30]),
            "qstart": pa.array([1, 1, 1]),
            "qend":   pa.array([8, 8, 8]),
            "tstart": pa.array([1, 1, 1]),
            "tend":   pa.array([7, 7, 7]),
            "cigar":  pa.array(["6M1I1M", "6M1I1M", "6M1I1M"]),
            "aligned_to": pa.array(["reference", "reference", "reference"]),
        }
    )
    abundance = {"P_TOP": 100.0, "P_MID": 10.0, "P_LOW": 1.0}
    result = classify_novel_peptides(
        detected, alignments, reference, novel,
        protein_abundance=abundance,
    )
    ...
    # transcript_id is the winning ORF.
    assert row["transcript_id"] == "P_TOP"
    # The protein_id column matches transcript_id (1:1 in this workflow).
    assert row["protein_id"] == "P_TOP"
    # All three ORFs supported this peptide.
    assert row["n_transcripts_supporting"] == 3
    # The list contains all three (order preserved from row-iteration).
    supporters = set(row["supporting_transcripts"].split(";"))
    assert supporters == {"P_TOP", "P_MID", "P_LOW"}


def test_single_supporting_transcript_when_only_one_orf() -> None:
    """A peptide supported by exactly one ORF still gets the columns
    populated: count=1, list=[protein_id]."""
    pep = "PEPTIDEK"
    detected = pa.table({"peptide_sequence": pa.array([pep])})
    novel = pa.table(
        {
            "protein_id": pa.array(["N_ONLY"]),
            "sequence": pa.array([pep]),
        }
    )
    reference = pa.table(
        {
            "protein_id": pa.array(["R1"]),
            "sequence": pa.array(["PEPTIDK"]),
        }
    )
    alignments = pa.table(
        {
            "query":  pa.array(["N_ONLY"]),
            "target": pa.array(["R1"]),
            ...
            "tstart": pa.array([1]),
            "tend":   pa.array([7]),
            "cigar":  pa.array(["6M1I1M"]),
            "aligned_to": pa.array(["reference"]),
        }
    )
    result = classify_novel_peptides(detected, alignments, reference, novel)
    rows = result.to_pylist()
    assert len(rows) == 1
    assert rows[0]["transcript_id"] == "N_ONLY"
    assert rows[0]["n_transcripts_supporting"] == 1
    assert rows[0]["supporting_transcripts"] == "N_ONLY"


def test_swissprot_aligned_peptide_gets_gene_populated() -> None:
    """Regression for the combined.fasta bug: when a novel peptide
    aligns to a SwissProt target (target not in the reference
    proteome → classification 'non_reference'), the gene column must
    still be populated from gene_map if the SwissProt accession is a
    key in that map.

    Mirrors the production setup: gene_map is the merged refseq
    annotation map + ``protein_to_gene_from_swissprot(swissprot.fasta)``
    output, the latter keyed on the bare UniProt accession that
    mmseqs2 reports as ``target``."""
    pep = "WAVLPPGRK"
    detected = pa.table({"peptide_sequence": pa.array([pep])})
    novel = pa.table(
        {
    ...
    # SwissProt target — NOT in the reference proteome.
    reference = pa.table(
        {
            "protein_id": pa.array(["REF1"]),
            "sequence": pa.array(["UNRELATEDPROTEINSEQUENCE"]),
        }
    )
    alignments = pa.table(
        {
            "query":  pa.array(["NOVEL1"]),
            "target": pa.array(["P04483"]),  # bare SwissProt accession
            "evalue": pa.array([1e-30]),
            "qstart": pa.array([1]),
            "qend":   pa.array([9]),
            "tstart": pa.array([1]),
            "tend":   pa.array([9]),
            "cigar":  pa.array(["9M"]),
            "aligned_to": pa.array(["swissprot"]),  # upstream-tagged
        }
    )
    # gene_map mirrors the orchestrator's merged RefSeq + SwissProt map.
    gene_map = {"P04483": "TETR2"}
    result = classify_novel_peptides(
        detected, alignments, reference, novel,
        gene_map=gene_map,
    )
    ...
    assert len(rows) == 1
    row = rows[0]
    # Classification short-circuits to non_reference because target ∉ refseq.
    assert row["classification"] == "non_reference"
    # *** The fix ***: gene name is populated from the merged map.
    assert row["gene"] == "TETR2"
    assert row["ref_protein_id"] == "P04483"
