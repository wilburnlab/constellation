...
- **Phase 3** — SHIPPED, with two deliberate deviations from this plan (both validated on synthetic panels):
  1. **Connected-components grouping, not radius-1 abundance-anchored greedy set-cover.** The greedy design assumes a dominant high-abundance variant to anchor each cluster. At ~1% per-base error over ~1 kb reads, almost every read is a *distinct* unique sequence (exact-derep abundance ≈ 1), so there is no abundance hub and radius-1 greedy under-clusters — a single transcript fragments into many clusters. Grouping by connected components of the edit-distance-verified graph collapses all minor variants together (two reads join only through a chain of ≥ identity-threshold edges; distinct transcripts > 2% divergent share no edge). The centroid is then the highest-abundance/longest component member, and the consensus aligns each member to it (reusing the cached verify CIGAR for directly-verified edges, re-aligning only component-path members). `scipy.sparse.csgraph.connected_components` does the grouping in C.
  2. **Per-cluster variant + haplotype + error-model analysis lands now** (the roadmap had deferred this to Layer 2). The PI wants the "what was collapsed" view as a core round-1 deliverable: a context-aware (homopolymer-run-length / substitution / non-hp-indel) depth-gated binomial/beta-binomial variant caller with a `--error-model empirical` fit, plus a read×variant-position haplotype collapse + pairwise-r² phasing and an MSA-free haplotype-map SVG. It does **not** re-cluster (that's Layer 2); it surfaces the structure so the user can judge whether a split is warranted.

  Lives in `constellation/sequencing/transcriptome/cluster/denovo/` (note: under `cluster/` per the PR-7 reorg, not the pre-reorg `cluster_denovo/`). The threshold caveat: `--identity` is a read-to-read gate, and two reads each ~1% from truth are ~2% apart, so ~0.96-0.97 reduces low-abundance-tail fragmentation for ~1% error reads; consensus-anchored re-clustering (`max_cluster_rounds`>1) is the principled fix and remains reserved.
- **EM round loop** — SHIPPED (`transcriptome cluster --mode em-orf` / `--mode em-kmer`). The ORF-anchored EM's stages had all shipped and nothing iterated them; `em/rounds.py` is the engine, with a calibrated assignment rule (absolute identity floor; round 1 on ORF replication + seed quality, round 2+ on a context-aware log-likelihood over the differing positions), no prune and no merge, and a mmapped row-indexed data architecture. See the EM-loop section of [constellation/sequencing/CLAUDE.md](../../constellation/sequencing/CLAUDE.md). Validated on synthetic panels; **not yet run at 9.4M scale** — the two numbers that decide the design are the `-N 500` saturation fraction and round-1 E-step wall time. **Round-1 seeding now has a second implementation** — `--mode em-kmer` runs the `--mode kmer` kernels on the reads instead of seeding per distinct ORF, which the bench measured at 4.4x fewer templates and a 2.89 h round-1 E-step against 13.95 h, with the `-N` cap-hit rate falling from 11.0% to 0.05%. See [em-kmer-seeding.md](em-kmer-seeding.md); the default remains `em-orf` until the multi-round comparison runs.
- **Phase 4** — pending. Cross-validation harness comparing Phase 3 against Phase 2 ground truth, plus `max_cluster_rounds`>1 consensus-anchored re-clustering (the principled fix for the read-to-read vs read-to-consensus identity-gate gap).
