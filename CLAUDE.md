...
| `constellation.viz` | First-party visualization layer. PR 1: `constellation viz genome --session DIR` (IGV-style focused tool) — FastAPI server, Apache Arrow IPC over HTTP, SVG-only client rendering with vector save, six track kernels (`reference_sequence`, `gene_annotation`, `coverage_histogram`, `read_pileup`, `cluster_pileup`, `splice_junctions`), Datashader-backed hybrid mode for dense layers. PR 2 (lean v1 shipped): bare `constellation` (or `constellation dashboard`) opens a JupyterLab-style dashboard (dockview-core, vanilla TS) wrapping every CLI subcommand as an auto-introspected soft GUI form + xterm.js terminal, with single-compute-job lock + WS stdio stream. Deferred follow-ups: embedded IPython panel, sandboxed FilePicker, embedded genome browser as a dock panel, desktop shortcut generator | partial (PR 1 + 2 shipped) | [constellation/viz/CLAUDE.md](constellation/viz/CLAUDE.md) |
