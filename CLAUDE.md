...
| `constellation.massspec.peptide` | Peptide-level MS chemistry: `precursor_mz`, `IonType` (a/b/c/x/y/z) + `fragment_ladder` returning paired Arrow `FragmentIonTable` + `(L-1, n_types, n_charges, n_losses+1)` float64 tensor with NaN-masked invalid losses, `peptide_envelope` (charge-aware binned + exact, with heavy-isotope `mass_override` shift), `LOSS_REGISTRY` (H2O / NH3 / HPO3 / H3PO4) with biochem-rule predicates (residue + modification triggers + ion-type restriction). Verified b/y parity vs Cartographer's `peptide_ladder_generator`. Pure physics — no model-specific decisions. | **shipped** |
...
| `constellation.massspec.search` | Search-engine output sibling to library/quant — PSMs, scores, q-values, FDR. **Scaffold only:** namespace reserved + module docstring listing planned schemas (PSM_TABLE, peptide-level / protein-level rollups). Schema design is held until the first concrete consumer arrives (EncyclopeDIA wrapper via `thirdparty.encyclopedia`, or the Counter port) so engine-specific column choices don't calcify the wrong contract. The `Search` placeholder constructor raises `NotImplementedError`. | scaffold |
...
- **Spectral libraries split theoretical from empirical.** `massspec.library` carries the protein → peptide → precursor → fragment DAG plus the only true M:N edge (protein ↔ peptide) and entity-level *predicted* properties (RT / CCS / fragment intensity) — sample-agnostic, sequence-derivable. `massspec.quant` carries per-acquisition observations (Protein/Peptide/PrecursorQuant) and *calibrated* per-edge transmission efficiencies (`TransmissionProteinPeptide`, `TransmissionPeptidePrecursor`); `efficiency` is strictly bounded `(0, 1]` on populated rows with `-1.0` as the uncalibrated sentinel. `massspec.search` (scaffold) holds detection results — PSMs, q-values — and is the third sibling. Run provenance lives once in `massspec.acquisitions` (`ACQUISITION_TABLE` + `Acquisitions` container + `validate_acquisitions` cross-check); both Search and Quant reference `acquisition_id`. A higher-level composition layer that unifies Library + Search + Quant per Constellation's "detection-IS-quantification" philosophy is intentionally deferred until a concrete pipeline drives its API. Library/Quant ship `LibraryReader` / `LibraryWriter` and `QuantReader` / `QuantWriter` Protocols + a registry; `ParquetDirReader/Writer` is the native lossless form, `DlibReader/Writer` and `ElibQuantReader/Writer` are stubbed (raise `NotImplementedError` with a pointer to cartographer's `encyclopedia.py`) pending the EncyclopeDIA reader port.
...
|---|---|---|
| ~~`cartographer/masses.py`~~ | ~~`core.chem.{elements,composition,isotopes,modifications}` + `core.sequence.alphabets` + `massspec.peptide`~~ | **Done** — `core.chem`, `core.sequence`, and `massspec.peptide` shipped. The fragment-ladder / m/z / neutral-loss-mask machinery (`peptide_ladder_generator`, `generate_neutral_loss_mask`, `calculate_peptide_mz`) is now `massspec.peptide.{ions, mz, neutral_losses}` — clean rewrite with `IonType` enum (full b/y/c/z + a/x; was b/y only), composition-arithmetic-derived ION_OFFSETS (no hardcoded constants), Arrow `FragmentIonTable` + tensor dual output (was tensor-only), JSON-vendored `LOSS_REGISTRY` (was Python dict; per CLAUDE.md data-file pattern), and the `[nterm]`/`[cterm]` legacy tokens dropped (position-0/L-1 mods on stripped sequence per `parse_modified_sequence` convention). Cartographer's `RESIDUE_REGISTRY` / `VOCABULARY` / `TOKEN_TO_INDEX` is **not** carried forward — that's per-model encoding, scheduled for `constellation.massspec.tokenize` (sibling to `peptide/`, scaffold-only this session) when the first model port begins. |
...
| `cartographer/data/encyclopedia.py` (sqlite + zlib + numpy decode) | `massspec.readers.encyclopedia` (TODO) + `massspec.{library, quant}` containers (**done**) | Library/quant containers shipped as the destination shape — `dlib` reads land in `Library` (proteins / peptides / precursors / fragments + protein-peptide edges), `elib` per-acquisition peptide quants land in `Quant` (`PEPTIDE_QUANT` rows + run-agnostic `TRANSMISSION_PROTEIN_PEPTIDE` calibration). `.dia` is raw spectra in sqlite (per-scan, not chromatograms) and lands in `massspec.readers.encyclopedia` next session, alongside the dlib/elib path. mzpeak deferred entirely until PSI ratifies — cartographer's "mzpeak" was an internal Parquet cache, not the standard. |
