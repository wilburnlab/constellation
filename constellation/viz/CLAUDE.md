...
| `viz.introspect` | PR 2: walks the production argparse parser via `walk_parser(_build_parser())` and emits a JSON tree the dashboard renders into auto-generated forms. `curated.json` is the hand-edited "Common" overlay (a `curated` array for the sidebar + an `arg_hints` array that refines per-argument path classification). Type mapping: `BooleanOptionalAction`/`store_true` → flag, `choices=` → enum, `nargs in ('+','*',N>1)` → multi, `dest` path-suffix heuristic → path. **File-picker layer:** path args also carry `path_kind` (`dir`/`file`/`either`, dir-vs-file split from the same dest heuristic, dir-first) so the frontend FilePicker opens in the right mode; repeatable path args stay `multi` but keep `path_kind` for a Browse-to-append affordance; `dest == "reference"` stamps `widget="reference"` (editable dropdown from `/api/references`, never a filesystem picker — guarded by `_NOT_PATH_DEST`); `arg_hints` (matched by `(path, dest)`) overrides `path_kind`/`widget`/`glob`/`is_path` for genuine heuristic misses (`--samples`→file `*.tsv`, `--reads`→either `*.bam,*.sam`, `--junc-bed`→file `*.bed`, `massspec inclusion-list --fasta`→file `*.fasta,*.fa,*.faa,*.fas` — `dest='fasta'` misses the walker's `endswith('_fasta')` test). | shipped |
...
| `viz.server.endpoints.fs` | `GET /api/fs/list` — sandboxed directory listing for the frontend FilePicker. Every path is confined to `app.state.fs_roots` (home + cwd + WSL `/mnt/*` drives by default; widened with `constellation dashboard --root DIR` / `viz genome --root DIR`). `_resolve_within_roots` normalizes pasted Windows paths (`C:\…`→`/mnt/c/…` via `wslpath`/regex under WSL), `resolve()`s to collapse `..` + deref symlinks, then a containment check rejects escapes (403); symlinks pointing out of the sandbox are dropped from listings. Query params: `path`, `dirs_only`, `globs`. | shipped |
...
- **Kernels don't import each other.** Each kernel module is a self-contained unit; shared helpers (contig-id lookup, greedy row assignment) are duplicated per-kernel rather than introducing a `tracks/_helpers.py`. Small price for module independence at this scale. **Exception: `tracks/_alignment_view.py`** ships the block-attach + cs:long mismatch-position helpers shared by `read_pileup` (PR 2) and (after PR 5) `cluster_pileup`'s member-view path — both kernels need the same ~150 LOC of nested-list assembly + cs grammar walk, so the trade flipped from "duplicate it" to "extract once." The helpers are underscore-prefixed (`_alignment_view`) to signal they're not part of the public kernel surface.
- **Viz reads from sequencing's cs:long parser directly** at runtime — `tracks/_alignment_view.py` imports `constellation.sequencing.align.cigar.parse_cs_long_mismatch_positions` to extract per-base substitution positions for the read_pileup track's X glyphs. This is the first viz → sequencing import; the precedent is `sequencing/align/consensus.py`'s internal use of the same parser. The cs grammar is non-trivial enough that duplicating the regex walk in viz would invite drift; routing through one canonical parser is the right trade. New cross-package imports should be similarly justified (pure function, narrow surface, no upstream state).
...
Fixture sessions are built in tmp_path: populate a fake reference cache root (via `monkeypatch.setenv("CONSTELLATION_REFERENCES_HOME", tmp_path / "refs")`) with `<organism>/<release_slug>/genome/{contigs.parquet,sequences.parquet}` + `meta.toml`, then write each source dir with a schema-v3 `manifest.json` referencing the cached handle plus its per-stage parquet artifacts. `Session.open(reference_handle=..., sources=[...])` builds the in-memory Session.
...
- `frontend/src/dashboard/NotebookPanel.ts` — IPython-in-dock; would reuse the `viz.runner` plumbing.
