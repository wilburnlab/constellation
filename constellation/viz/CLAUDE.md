...
- `tests/test_viz_saved_sessions.py` — Saved-session cache CRUD + endpoint lifecycle. 8 tests.
...
The full viz suite (97 tests) runs in <6s; the slow e2e takes ~1.4s extra. Tests gate on `pytest.importorskip("fastapi" / "datashader")` so they don't pollute the base-install smoke run.
...
└── src/
    ...
    │   ├── GenomeBrowser.ts       toolbar (zoom +/–/Fit / Labels / search / Datasets popover /
    │   │                          Save SVG) + overview bar + ruler + track stack with per-track
    │   │                          drag-grip / collapse / hide / bottom-edge resize controls;
    │   │                          owns per-binding visible/displayOrder/heightPx/collapsed state
    │   │                          and persists it to localStorage + saved-session TOML
...
- **Per-track aesthetic styling** — gear-icon menu for color / palette / opacity / label-override / per-sample filter. PR 5 explicitly scoped this out because the underlying per-kernel visualizations themselves are likely to be rescoped soon; styling controls should land alongside the next round of renderer work so they target the final visual vocabulary, not the v1 one. Per-binding `visible` / `displayOrder` / `heightPx` / `collapsed` already exist as plumbing — color/palette would extend the same `TrackLayoutEntry` shape and PATCH path.
