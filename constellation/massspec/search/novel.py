...
priority. Three meaningful differences from the cartographer original:
  ...
  2. **Alignment tier is always (re)computed from target membership in
     ...
     internally from membership. Any ``aligned_to`` column already
...
def classify_single_peptide(
    ...
    hit_row
        ...
        ``aligned_to``. Coordinates are 1-indexed inclusive (mmseqs2
        convention). ``aligned_to == "non_reference"`` (or any string
    ...
    """
    if str(hit_row["aligned_to"]) != "reference":
...
def _select_best_hit_per_query(alignments: pa.Table) -> pa.Table:
    ...
    ``aligned_to == "reference"`` hits**. This matters when the
    ...
    Falls back to e-value-only ordering when the ``aligned_to``
    ...
    if "aligned_to" in alignments.column_names:
        # tier_rank = 0 for reference, 1 otherwise → reference wins ties.
        tier_rank = pc.if_else(
            pc.equal(alignments.column("aligned_to"), "reference"),
...
def _infer_aligned_to(
    alignments: pa.Table, reference_protein_ids: set[str]
) -> pa.Table:
    """Compute the canonical ``aligned_to`` from target membership in
    ...
    **Any incoming ``aligned_to`` column is replaced.** Upstream
    ...
    if "aligned_to" in alignments.column_names:
        return alignments.set_column(
            alignments.column_names.index("aligned_to"),
            "aligned_to",
            tier_arr,
        )
    return alignments.append_column("aligned_to", tier_arr)
...
def classify_novel_peptides(
    ...
    *,
    ...
    alignments
        ...
        hit per query. ``aligned_to`` may be null on input — see the
    ...
    alignments_tiered = _infer_aligned_to(alignments, reference_protein_ids)
    ...
    #
    # Before the tie-break, capture per-peptide-sequence the set of all
    # source ORFs that produced this peptide. These populate the
    # `transcript_id` (winning ORF), `n_transcripts_supporting`, and
    # `supporting_transcripts` (semicolon-joined list) columns so the
    # multi-mapping information isn't lost after the most-abundant pick.
    seq_supporters: dict[str, list[str]] = {}
    for r in rows:
        seq = r["peptide_sequence"]
        pid = r.get("protein_id")
        if not isinstance(pid, str):
            continue
        supporters = seq_supporters.setdefault(seq, [])
        if pid not in supporters:
            supporters.append(pid)
    ...
    for seq, verdict in winning.items():
        ...
        supporters = seq_supporters.get(seq, [])
        winning_pid = verdict.get("protein_id")
        transcript_id = (
            str(winning_pid) if isinstance(winning_pid, str) else None
        )
        n_supporting = len(supporters) if supporters else None
        supporting = ";".join(supporters) if supporters else None
        for modseq in modforms:
            row = dict(verdict)
            row["modified_sequence"] = modseq
            row["transcript_id"] = transcript_id
            row["n_transcripts_supporting"] = n_supporting
            row["supporting_transcripts"] = supporting
